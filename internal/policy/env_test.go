...
func TestComposeEnvEntriesAllExcept(t *testing.T) {
	...
	entries, err := composeEnvEntries(nil, parent, []string{"SECRET"}, nil, []string{"EXTRA=1"})
	if err != nil {
...
func TestComposeEnvEntriesEnvFile(t *testing.T) {
	...
	entries, err := composeEnvEntries([]string{"HOME"}, map[string]string{}, nil, []string{file}, nil)
	if err != nil {
		t.Fatalf("composeEnvEntries: %v", err)
	}
