---
title: Denial Diagnostics
description: How bulle reports sandbox denials with copy-pasteable policy fixes.
hide:
  - navigation
---
...
These hints are diagnostics only. Enforcement is always done by the kernel ([Landlock](https://landlock.io/) on Linux, Seatbelt on macOS) and works identically whether or not denial logging is available.
...
1. **A kernel with Landlock audit support** — Linux 6.15 or newer (Landlock ABI v7). On older kernels, `bulle` enforces the sandbox exactly the same but cannot obtain denial records.
2. **The audit subsystem enabled at runtime.** Some distributions ship it on, others off:
...
!!! warning "Hints are suggestions, not automatic decisions"

    A denial means the sandbox worked. Before copying a suggested flag, consider whether the command *should* have that access — an untrusted tool probing `~/.ssh` is exactly what the sandbox is for.
